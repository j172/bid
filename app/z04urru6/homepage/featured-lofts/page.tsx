...
// 名家專區 (see GitHub issue #168) — a second admin page built on top of the
// same generic homepage_sections CRUD from #33, running fully independent of
// 合作鴿舍 (issue #34/partner-lofts) via its own section_type constant. Kept
// to a single section_type constant here rather than importing one from
// lib/homepageSections.ts because that module deliberately stays
...
export default async function FeaturedLoftsAdminPage() {
  ...
  return (
    <main>
      <AdminPageIntro title="名家專區管理" description="管理首頁與商品列表頁「名家專區」區塊的卡片：圖片、簡介與排序。卡片點擊後會導向該名家的商品列表。停用後會立即從前台隱藏，不需重新部署。">
        <FeaturedLoftFormModal mode="create" sectionType={SECTION_TYPE} />
      </AdminPageIntro>
