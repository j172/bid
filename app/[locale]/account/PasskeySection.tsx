...
export default function PasskeySection({ initialPasskeys }: { initialPasskeys: PasskeyListItem[] }) {
  ...
  // `registering`/`deletingId` stay local rather than reusing the hook's own
  // in-flight flag: unlike the account page's other sections, an add and a
  // delete can be visible at the same time, so one shared flag would disable
  // the "新增" button while an unrelated row is being deleted.
  ...
  async function handleStartRegistration() {
    setRegistering(true);
    try {
      const optionsData = await post<{ ok?: boolean; options: Parameters<typeof startRegistration>[0]["optionsJSON"] }>(
  ...
  async function handleNameSubmit(event: React.FormEvent) {
    ...
    const data = await post<{ ok?: boolean; passkey: PasskeyListItem }>("/api/account/webauthn/register-verify", {
  ...
  async function handleDelete(credentialId: string) {
    ...
    const data = await post("/api/account/webauthn/delete", { credentialId });
