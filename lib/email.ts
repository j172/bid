...
// The Resend half of an outbound call: auth header, JSON body, and the
// api.resend.com base URL. The node:https transport underneath (and why it
// isn't fetch()) lives in lib/httpsRequest.ts.
//
// Shared by every Resend caller (transactional email, audience signup,
// broadcasts) so they all build the same headers from one place.
export function resendRequest(method: string, path: string, body?: unknown): Promise<{ status: number; body: string }> {
  ...
  return httpsRequest(`https://api.resend.com${path}`, { method, headers, body: payload });
}
