...
export function buildLlmsFullTxt(): string {
  const lines = [
    ...
    `- 競標與商品列表 [Listings](${absoluteUrl("/listings")}): 所有拍賣與一口價商品，可依分類篩選 (\`?type=auction\` 或 \`?type=fixed_price\`) 與名家鴿舍篩選；頁面底部設有「名家專區」精選鴿舍卡片區塊。`,
...
export function buildLlmsTxt(): string {
  const lines = [
    ...
    `- [Listings](${absoluteUrl("/listings")}): every open auction and fixed-price listing, filterable by ` +
      `category (\`?type=auction\` or \`?type=fixed_price\`); the page footer features a curated "featured lofts" card grid.`,
